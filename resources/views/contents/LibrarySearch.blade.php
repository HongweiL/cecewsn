...
@section('content')
    <nav class="top-bar">
        <a id="method">Process method</a>
        <a id="choice" class="current-status">Choose files</a>
        ...
        <section id="upload-file">
            <div class="layui-container" style="margin-top: 20px; margin-bottom: 20px">
                <div class="layui-row">
                    ...
                    <div class="layui-col-md3">
                        <label>Adducts: </label>
                        <span id="file-adducts-choice">No file chosen</span>
                        <br>
                        <label class="layui-btn" style="background-color: #2c3f52" for="file-adducts">Choose</label>
                        <input id="file-adducts" onchange="change(this.id)" class="uploadBtn" type="file" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" style="position:absolute;clip:rect(0 0 0 0);"/></br>
                    </div>
                </div>
            </div>
            <table>
                ...
                <tr>
                    <td>Source = needs description</td>
                    <td><input id="source" type="text" placeholder="Please enter source" onkeypress="return (event.charCode >= 65 && event.charCode <= 90) || (event.charCode >= 97 && event.charCode <= 122) || (event.charCode >= 48 && event.charCode <= 57)"></td>
                </tr>
                <tr>
                    <td>Mode = needs description</td>
            ...
            <div class="layui-row">
                <div class="layui-col-md5">
                    <div style="color:#C8C8C8; font-family: monospace; background-color: black; border: 3px groove #ccc; width: 90%; height:80vh; margin-top: 5%; padding: 5px">
                        ...
                        <p id="low-name">please choose low energy file before upload</p>
                        <p id="adducts-name">please choose adducts file before upload</p>
                        <p id="d_set">no parameter</p>
                        <p id="source_t">no source</p>
                        <p id="mode_t">mode not selected</p>
                        <p id="prog1"></p>
                        <p id="prog2"></p>
                        <p id="prog3"></p>
                        <p id="prog4"></p>
    ...
    <script>
        ...
        var adducts = document.getElementById('file-adducts');
        ...
        button.addEventListener('click', function () {
            ...
            if (file_target && file_high && file_low) {
                ...
                const args = $("input[id='W']").val() + "="
                    ...
                    + $("input[id='S2N']").val() + "\n" + method + "\n" + $("select[id='mode']").val() + "-splitHere-" + $("input[id='source']").val();
                ...
                const file4 = {Key: dir+"adducts/"+file_adducts.name, ContentType: file_adducts.type, Body: file_adducts, 'Access-Control-Allow-Credentials': '*','ACL': 'public-read'};
                ...
                bucket.upload(file1).on('httpUploadProgress', function(evt) {
                    prog1.innerHTML = ("1/4 Uploading: " + parseInt((evt.loaded * 100) / evt.total)+'%');
                ...
                bucket.upload(file2).on('httpUploadProgress', function(evt) {
                    prog2.innerHTML = ("2/4 Uploading: " + parseInt((evt.loaded * 100) / evt.total)+'%');
                }).send(function(err, data) {
                    if (err) {
                        results.innerHTML = err;
                    } else {
                ...
                bucket.upload(file3).on('httpUploadProgress', function(evt) {
                    prog3.innerHTML = ("3/4 Uploading: " + parseInt((evt.loaded * 100) / evt.total)+'%');
                }).send(function(err, data) {
                    if (err) {
                        results.innerHTML = err;
                    } else {
                ...
                bucket.upload(file4).on('httpUploadProgress', function(evt) {
                    prog4.innerHTML = ("4/4 Uploading: " + parseInt((evt.loaded * 100) / evt.total)+'%');
                }).send(function(err, data) {
                    if (err) {
                        results.innerHTML = err;
                    } else {
        ...
        function goTo3() {
            ...
            var file_low = low.files[0];
            var file_adducts = adducts.files[0];
            if (file_target && file_high && file_low && file_adducts) {
                if ($("input[id='W']").val()!='' && $("input[id='min_int']").val()!='' && $("input[id='mass_tol']").val()!='' && $("input[id='R_min']").val()!='' && $("input[id='P_max']").val()!='' && $("input[id='r_t']").val()!='' && $("input[id='ms_w']").val()!='' && $("input[id='ms_p_w']").val()!='' && $("input[id='Mass_w']").val()!='' && $("input[id='S2N']").val()!='' && $("select[id='mode']").val()!='' && $("input[id='source']").val()!='') {
                    $("#2").hide();
                    $("#choice").attr("class", "");
                    $("#3").show();
        ...
        function goTo4() {
            ...
            $("#adducts-name").html("add_: "+file_adducts.name);
            $("#source_t").html("src_: "+$("#source").val());
            $("#mode_t").html("mode: "+$("#mode").val());
